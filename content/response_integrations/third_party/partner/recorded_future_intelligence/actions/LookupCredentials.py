...
def main():
    ...
    subjects = param_validator.validate_csv(
        param_name="subjects",
    ...
    try:
        ...
        siemplify.LOGGER.info("Searching credentials for given subject(s)")
        lookup_resp = identity_mgr.lookup_credentials(
            subjects=subjects,
        ...
        output_message = f"Error searching subject credentials: {err}"
        siemplify.LOGGER.error(output_message)
        is_success = False
        status = EXECUTION_STATE_FAILED

    except Exception as err:
        output_message = f"Error executing Lookup Credentials action: {err}"
        is_success = False
        status = EXECUTION_STATE_FAILED
