...
from psengine.identity import IdentityMgr, IdentitySearchError
...
from ..core.constants import CSV_DELIMETER, DOMAIN_TYPES, PROVIDER_NAME
...
def main():
    ...
    domains = param_validator.validate_csv(
        param_name="domains",
    ...
    domain_types = param_validator.validate_csv(
        param_name="domain_types",
        csv_string=extract_action_param(
            siemplify,
            param_name="Domain Types",
            input_type=str,
            is_mandatory=False,
            print_value=True,
            remove_whitespaces=True,
    ...
    try:
        ...
        output_message = f"Error searching credential data: {err}"
        siemplify.LOGGER.error(output_message)
        is_success = False
        status = EXECUTION_STATE_FAILED

    except Exception as err:
        output_message = f"Error executing Search Credentials action: {err}"
        is_success = False
        status = EXECUTION_STATE_FAILED
